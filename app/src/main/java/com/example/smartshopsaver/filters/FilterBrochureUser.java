...
public class FilterBrochureUser extends Filter {
     ...
     */
    public FilterBrochureUser(AdapterBrochureUser adapter, ArrayList<ModelBrochure> filterList) {
        ...
        if (constraint != null && constraint.length() > 0) {
            ...
            for (int i = 0; i < filterList.size(); i++) {
                ...
                if (filterList.get(i).getBrochureName().toUpperCase().contains(constraint) ||
                        filterList.get(i).getBrochureCategory().toUpperCase().contains(constraint)
                ) {
