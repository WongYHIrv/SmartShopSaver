...
public class FilterBrochureCategoryAdmin extends Filter {
     ...
     */
    public FilterBrochureCategoryAdmin(AdapterBrochureCategoryAdmin adapter, ArrayList<ModelBrochureCategory> filterList) {
    ...
    protected FilterResults performFiltering(CharSequence constraint) {
        ...
        if (constraint != null && constraint.length() > 0) {
            ...
            ArrayList<ModelBrochureCategory> filteredModels = new ArrayList<>();
            for (int i = 0; i < filterList.size(); i++) {
                //Ad filter based on Brand, Category, Condition, Title. If any of these matches add it to the filteredModels list
                if (filterList.get(i).getBrochureCategory().toUpperCase().contains(constraint)) {
    ...
    protected void publishResults(CharSequence constraint, FilterResults results) {
        ...
        adapter.brochureCategoryArrayList = (ArrayList<ModelBrochureCategory>) results.values;
